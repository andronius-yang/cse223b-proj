...
Scenario configs should not schedule rank events after all request streams have completed. Such events are outside the modeled inference and should be rejected as configuration errors.
...
Completed request streams are inert. Later rank events can affect expert state on their source ranks, but completed streams do not pause, resume, or produce more traffic.
