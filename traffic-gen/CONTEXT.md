...
**Blocked Request Stream**:
A live request stream whose next layer expert has no live resident replica. It does not emit partial traffic or advance until a later event restores expert availability.
_Avoid_: Failed request, skipped expert, partial route
...
The live replica chosen as the source for recreating a missing layer expert replica during repair. The MVP prefers a live replica on the destination node, otherwise the first live replica found by circular rank search from the destination rank.
...
**Expert Disk IO**:
Storage-based expert state restoration used only when join repair cannot find any live source replica for the missing layer expert. It is mutually exclusive with expert migration for the same missing replica.
_Avoid_: Network migration, synthetic live replica, terminal repair failure

**Lost Expert Bytes**:
The amount of expert state restored from storage because no live replica survived for the missing layer expert. These bytes are not rank-to-rank traffic.
_Avoid_: Migration bytes, traffic matrix bytes, cold-start matrix
...
A layer expert needed by inference for which no live resident replica exists in the current step. The affected request stream blocks rather than emitting partial traffic.
_Avoid_: Dropped expert, synthetic replica, silent token skip

**Terminal Scenario Failure**:
An explicit scenario-ending failure record emitted when generation cannot make progress and no future event can restore progress. Partial outputs before the terminal failure remain valid for inspection, but the scenario itself is invalid.
...
**Lazarus Placement Target**:
The planned layer-expert replica placement produced by the Lazarus allocator and placement policy. Traffic generation treats it as the sole placement truth and does not pin baseline owners.
_Avoid_: Scenario-local placement, owner-pinned placement, event-local reshuffle
